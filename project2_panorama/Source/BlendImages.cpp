...
static void AccumulateBlend(CByteImage& img, CFloatImage& acc, CTransform3x3 M, float blendWidth)
{
    ...
    for (int y = bb_min_y; y <= bb_max_y; y++) {
        for (int x = bb_min_x; x < bb_max_x; x++) {
            ...
            double weight_x=1-((double)(1+abs( x-(bb_min_x+bb_max_x)/2) ) /(double)(1+(bb_min_x+bb_max_x)/2));
            double weight_y=1-((double)(1+abs( y-(bb_min_y+bb_max_y)/2) ) /(double)(1+(bb_min_y+bb_max_y)/2));
            
            weight=weight_x*weight_y;

			acc.Pixel(x, y, 0) += (float) (weight * img.PixelLerp(x_src, y_src, 0)/255);
            acc.Pixel(x, y, 1) += (float) (weight * img.PixelLerp(x_src, y_src, 1)/255);
            acc.Pixel(x, y, 2) += (float) (weight * img.PixelLerp(x_src, y_src, 2)/255);
...
static void NormalizeBlend(CFloatImage& acc, CByteImage& img)
{
    ...
    for(int i=0;i<sh.width;i++){
        for(int j=0;j<sh.height;j++){
            float imgp0=acc.Pixel(i, j, 0)/(acc.Pixel(i, j, 3));
            float imgp1=acc.Pixel(i, j, 1)/(acc.Pixel(i, j, 3));
            float imgp2=acc.Pixel(i, j, 2)/(acc.Pixel(i, j, 3));
            float base=1;
            if(acc.Pixel(i, j, 4)==0)continue;
            img.Pixel(i, j, 0)=(255.0*(imgp0/base));
            img.Pixel(i, j, 1)=(255.0*(imgp1/base));
            img.Pixel(i, j, 2)=(255.0*(imgp2/base));
...
CByteImage BlendImages(CImagePositionV& ipv, float blendWidth)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        int minx;int miny;int maxx;int maxy=0;
        ImageBoundingBox(ipv[i].img, ipv[i].position, minx,miny,maxx, maxy);
    ...
    A[1][2]=-((y_init-y_final)/(x_final-x_init))*(0.5 * width);
    A[1][0]=-(y_init-y_final)/(x_final-x_init);
    A[1][1] = 1;
