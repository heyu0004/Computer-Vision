              ...
              int nRANSAC, double RANSACthresh, CTransform3x3& M)
{
    ...
    for(int i=0;i<nRANSAC;i++){
        CTransform3x3 Mit;
        FeatureMatch selectpair=matches[randomset[i]];
        int id1=selectpair.id1;
        int id2=selectpair.id2;
        Mit[0][2]=f2[id2].x-f1[id1].x;
        Mit[1][2]=f2[id2].y-f1[id1].y;
        
    
    int score=countInliers(f1,f2,matches,m, f,Mit,  RANSACthresh, inliers);
        if (score>maxscore){
            M=Mit;
            Bestinliers=inliers;
            maxscore=score;
                 ...
                 CTransform3x3 M, double RANSACthresh, vector<int> &inliers)
{
    ...
    for (unsigned int i=0; i<(int) matches.size(); i++) {
        ...
        if(!m){
            ...
            int d=abs(f1[id1-1].x+M[0][2]-f2[id2-1].x)+(f1[id1-1].y+M[1][2]-f2[id2-1].y);
            if(d<RANSACthresh){
                inliers.push_back(i);
                count++;
            }
                    ...
                    const vector<int> &inliers, CTransform3x3& M)
{
    ...
    for (int i=0; i<inliers.size(); i++) {
        ...
        xTrans=f2[matches[i].id2-1].x-f1[matches[i].id1-1].x;
        yTrans=f2[matches[i].id2-1].y-f1[matches[i].id1-1].y;
        // END TODO
